...
if( !is_admin()){ 
	$jquery_url = 'http://cdnjs.cloudflare.com/ajax/libs/jquery/1.11.0/jquery.min.js';
	$check_url = @fopen($jquery_url,'r');
	if($check_url !== false) {
	    function wpbase_load_jquery() {
			wp_deregister_script('jquery');
			wp_register_script('jquery', '//cdnjs.cloudflare.com/ajax/libs/jquery/1.11.0/jquery.min.js', array(), '1.11.0', true);
...
function wpbase_scripts() {
	...
	wp_enqueue_script('modernizr', get_template_directory_uri() . '/assets/js/modernizr-2.7.0.min.js', array(), null, true);
	wp_enqueue_script('functions', get_template_directory_uri() . '/assets/js/functions.js', array(), null, true);
